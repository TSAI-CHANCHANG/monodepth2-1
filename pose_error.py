...
def calRelativePose(poseMatrix1, poseMatrix2):
    """Calculate the relative pose FROM matrix1 To matrix2
    # Return Value: relativeMartix
    """
    ...
    relativeRotM = rotMatrix1.T @ rotMatrix2
    relativeTransV = transVec2 - transVec1
    relativeMatrix = np.zeros((4, 4), dtype=np.float64)

    relativeMatrix[0:3, 0:3] = relativeRotM
    relativeMatrix[0:3, 3] = relativeTransV
    relativeMatrix[3, 3] = 1
